...
module Emissary
  ...
  class Daemon
    ...
    attr_accessor :state, :name, :config, :config_file
    ...
    def initialize(name, opts = {})
      ...
      self.class.const_set('STARTUP_OPTS', opts.clone_deep) unless self.class.const_defined? 'STARTUP_OPTS'
      ...
      self.class.const_set('CONFIG_FILE', @config_file) unless self.class.const_defined? 'CONFIG_FILE'
    ...
    def self.get_config(config_file, opts = {})
      ...
      config[:general][:agents] = config[:general][:agents].instance_of?(String) ? config[:general][:agents].split(/\s*,\s*/) \
      ...
      config[:general][:pid_dir]  = config[:general][:pid_dir] || '/var/run'
      ...
      config[:general][:operators].each do |operator|
        config[operator.to_sym].each do |name,data|
          ...
          agents = data[:agents].blank?          ? config[:general][:agents] \
                 : data[:agents].kind_of?(Array) ? data[:agents] \
    ...
    def self.validate_config!(config)
      unless config[:general]
        raise ::Emissary::ConfigValidationError.new("Missing 'general' section in configuration file")
      end

      unless config[:general][:operators]
        raise ::Emissary::ConfigValidationError.new("[general][operators] not set")
      end

      unless config[:general][:operators].kind_of? Array
        raise ::Emissary::ConfigValidationError.new("[general][operators] not a list")
      end

      config[:general][:operators].each do |operator|
        operator = operator.to_sym
        unless config[operator]
          raise ::Emissary::ConfigValidationError.new("Missing Operator Section '#{operator}'")
        end

        unless config[operator].kind_of? Hash
          raise ::Emissary::ConfigValidationError.new("Operator Section '#{operator}' not a dictionary of operators")
        end
      end
      ...
      while not @shutting_down do
        @operators.each do |name,operator|
          ...
          if can_startup? operator
            ...
            operator[:daemon] = Servolux::Daemon.new(:server => operator[:server], :pid_file => server_data[:pid_file], :logger => ::Emissary.logger)
